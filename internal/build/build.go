	...
	defer func() {
		for _, clean := range mountClean {
			if cleanErr := clean(); cleanErr != nil {
				l.Error("Unmount failed: %v", cleanErr)
			}
		}
	}()
...
func createDisk(run sys.Runner, img image.Image, diskSize image.DiskSize) error {
	if diskSize == "" {
...
func setupDevice(run sys.Runner, img image.Image) (device string, err error) {
	losetup := "losetup"
...
func detachDevice(run sys.Runner, device string) error {
	losetup := "losetup"
